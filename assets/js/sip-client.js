...
function setSipStatus(text, dotClass) {
  ...
  window.dispatchEvent(
    new CustomEvent("sip:status", { detail: { text, dotClass } }),
  );
...
async function ensureMicrophonePermission() {
  if (!navigator.mediaDevices?.getUserMedia) {
    console.error(
      "Microphone permission unsupported: navigator.mediaDevices.getUserMedia is unavailable",
    );
    return "unsupported";
  }
...
function attachRemoteTrackHandler(
  session,
...
function addSessionEvents(session, handlers = {}) {
  ...
  sessionEvents.forEach((eventName) => {
    session.on(eventName, (event) => {
      ...
      if (
        ["ended", "failed", "bye", "rejected", "canceled"].includes(eventName)
      ) {
...
export async function initSipClient(config, handlers = {}) {
  ...
  sipUA.on("connecting", () => {
    ...
    setSipStatus("Conectando", "is-offline");
  });
  ...
  sipUA.on("registered", () => {
    registered = true;
    console.log("SIP registered", registered);
    setConnectionInfo("Registered1");
    setSipStatus("Conectado", "is-online");
...
export function registerSip() {
  ...
  setSipStatus("Registering2", "is-offline");
...
function getWebSocketState() {
  const socket =
    sipUA?._transport?.socket?._ws ||
    sipUA?._transport?.socket?.socket ||
    sipUA?._transport?.socket;
...
export function createOutgoingSession(number, eventHandlers = {}) {
  ...
  if (!target) {
    console.error("Cannot make SIP call: invalid target", {
      number,
...
export function blindTransferSession(session, targetNumber) {
  ...
  if (!target) {
    console.error(
      "Blind transfer failed: invalid target or SIP domain unavailable",
      {
        targetNumber,
        sipUri: sipConfig?.sipUri,
      },
    ...
    session.refer(target, {
      eventHandlers: {
        requestSucceeded: (event) =>
          console.log("Blind transfer REFER request succeeded", event),
        requestFailed: (event) => {
